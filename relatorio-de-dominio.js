...
const executar = async () => {
    const usuarios = await User.getUsersFromDomain({
        domain: "ccastilho.brastel.com.br"
    }).catch((error) => {
        console.log(error)
    })

    const terminais = await User.getTerminaisFromDomain({
        domain: "ccastilho.brastel.com.br"
    }).catch((error) => {
        console.log(error)
    })

    const dids = await DIDs.getDidsFromDomain({
        domain: "ccastilho.brastel.com.br"
    }).catch((error) => {
        console.log(error)
    })

    const grupos = await Groups.getGroupsFromDomain({
        domain: "ccastilho.brastel.com.br"
    }).catch((error) => {
        console.log(error)
    })
